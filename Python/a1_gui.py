...
class Clock(object):
    ...
    def __init__(self, tk, zone=None, **kargs):
        ...
        self.canvas = tkinter.Canvas(self.internal_frame, width=self.width, height=self.height)
        self.canvas.grid()
...
if __name__ == '__main__':        
    ...
    tkinter.Button(frame, text='Slow motion', command=slow).grid(row=3, column=0)
    ...
    tkinter.Button(frame, text='Normal speed', command=normal_speed).grid(row=3, column=1)
    ...
    # function as the one to be called when the button is clicked.
    tkinter.Button(frame, text='Fast forward', command=speed).grid(row=3, column=2)
